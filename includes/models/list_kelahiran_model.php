...
class List_Kelahiran_Model extends Model {
    ...
    public function getBabyCowKelahiran($kelahiran_id,$format="object") {
	    global $dcistem;
		$db   = $dcistem->getOption("framework/db");
        $master = new Master_Ref_Model();
		if(trim($kelahiran_id)=="" or  $kelahiran_id == null){
		    ...
		    $referensi	= $_SESSION["referensi"];
			$filter="kelahiran_id=".$kelahiran_id."";
            $data_qry= $db->select("id,kelahiran_id,cow_id,sex,no_eartag,posisi_eartag,name,berat,harga_jual,
            tanggal_lahir","keswan_kelahiran_bayi")->where($filter)->lim();
            $listdata=array();
            while($data = $db->fetchObject($data_qry))
            {
                ...
                $rec    	   = new stdClass;
                $rec->id            = $data->id;
                $rec->kelahiran_id  = $data->kelahiran_id;
                $rec->cow_id        = $data->cow_id;
                $rec->sex_id        = $data->sex;
                $rec->sex_name        = $referensi['sex_sapi'][$data->sex];
                $rec->no_eartag        = $data->no_eartag;
                $rec->posisi_eartag        = $data->posisi_eartag;
                $rec->koloni_name        = $data->name;
                 $rec->berat        = $data->berat;
                $rec->harga_jual        = $data->harga_jual;
                $rec->tanggal_lahir        = $master->detailtanggal($data->tanggal_lahir,2);
                if(trim($format)=="array"){
					$listdata[] = (array) $rec;
				}else{
					$listdata[]	= $rec;
