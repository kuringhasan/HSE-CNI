...
class Load_Factor_Model extends Model {
 ...
 public function update($id_lama,$nama,$contractor_id,$closed,$is_empty) {
    ...
    public function getLoadFactor($id,$format="object") {
	    global $dcistem;
		$db   = $dcistem->getOption("framework/db");
        $master = new Master_Ref_Model();
		if(trim($id)=="" or  $id == null){
			...
			$filter="id=".$id."";
            $data= $db->select("id,load_factor_expit,load_factor_barging,berlaku_mulai,DATE_FORMAT(berlaku_mulai,'%d/%m/%Y') berlaku_mulai2,
            berlaku_sampai,DATE_FORMAT(berlaku_sampai,'%d/%m/%Y') berlaku_sampai2,
            ifnull(closed,0) closed","load_factor")->where($filter)->get(0);
