...
class List_Partner_Model extends Model {
    ...
    public function getListActiveContractor(){ 
		global $dcistem;
       	$db = $dcistem->getOption("framework/db");
        $master		=new Master_Ref_Model();
       
      	$filter	="ifnull(is_contractor,'')='1' and ifnull(active,'active')='active'";
    
        $list_qry=$db->select("id,name,code,alias,rgb_color","partner")->where($filter)->orderby("id asc")->lim();//
        $ListData=array();
        while($data = $db->fetchObject($list_qry))
        {
            $ListData[$data->id]['id']=$data->id;
            $ListData[$data->id]['name']=$data->name;
            $ListData[$data->id]['code']=$data->code;
            $ListData[$data->id]['alias']=$data->alias;
            $ListData[$data->id]['rgb_color']=$data->rgb_color;
        }
     ...
     public function json($ketgori,$nama="",$other_value=array()) {
        ...
        switch($ketgori){
            case "list_partner":
            	$filter="( alias like '%".$nama."%' or name like '%".$nama."%')";
        	    $list_qry= $db->select(" id,no_induk,name,alias,gelar_depan,gelar_belakang","partner")->where($filter)->lim();
        		$i=0;
        		while($data=$db->fetchObject($list_qry)){
        		   // $nama2=str_ireplace($nama,"<strong>".$nama."</strong>",$data->lokNamaUPB);
                    ...
                    $lengkap=trim($data->alias)<>""?$nama_lengkap." (".$data->alias.")":$nama_lengkap;
        		    $List[$i]['Lengkap']="<div class='label_typeahead'>".$lengkap."</div>";
        		    $i++;
        		}
            break;
            case "list_contractor":
            	$filter="ifnull(is_contractor,'')=1 and ( alias like '%".$nama."%' or name like '%".$nama."%')";
        	    $list_qry= $db->select(" id,no_induk,name,alias,gelar_depan,gelar_belakang","partner")->where($filter)->lim();
        		$i=0;
        		while($data=$db->fetchObject($list_qry)){
        		   // $nama2=str_ireplace($nama,"<strong>".$nama."</strong>",$data->lokNamaUPB);
