...
class Ref_Tingkat_Keparahan_Controller extends Admin_Template_Controller {
	...
	public function index() {
        ...
        $tpl  = new View("ref_tingkat_keparahan");
           ...
           if(trim($proses)=="save")
           {
            //    echo json_encode("tes");  
               ...
               if(count($validasi['arrayerror'])==0){
                   $cek=$db->select("kode","ref_tingkat_keparahan")->where("kode='".$kode."'")->get(0);
                   if(empty($cek)){
                       ...
                       $cols="kode,keterangan";
                       $values="$kode_val,$keterangan_val";
                       $sqlin="INSERT INTO ref_tingkat_keparahan ($cols) VALUES ($values);";
			...
			$tpl  = new View("form_ref_tingkat_keparahan");
	...
	public function edit($id,$proses=""){  
        ...
        {    
                    ...
                    if(count($validasi['arrayerror'])==0){
                        $cek=$db->select("kode","ref_tingkat_keparahan")->where("kode='".$kode."'")->get(0);
                   		if(empty($cek) || $kode == $id){
							...
							$cols_and_vals="kode=$kode_val,keterangan=$keterangan_val";
						
							$sqlin="UPDATE ref_tingkat_keparahan SET $cols_and_vals WHERE kode=$id;";
            ...
            $partner=new Ref_Tingkat_Keparahan_Model();

            $tpl  = new View("form_ref_tingkat_keparahan");
            $detail=$partner->getDetailRefTingkatKeparahan($id);
            $tpl->detail = $detail;
    ...
    public function detail($id){     
		global $dcistem;
		$db   = $dcistem->getOption("framework/db");
		$tpl  = new View("detail_ref_tingkat_keparahan");
		$insidenMdl    = new Ref_Tingkat_Keparahan_Model();
		date_default_timezone_set("Asia/Jakarta");
		$detail=$insidenMdl->getDetailRefTingkatKeparahan($id);
		// $tpl->list_contractor =Model::getOptionList("partner","id","name","name ASC","ifnull(is_contractor,0)=1"); 

		$tpl->kode            		= $detail->kode;
		$tpl->keterangan       	= $detail->keterangan;
	...
	public function del($id){     
        ...
        if(trim($id)=="" or  $id == null){
            ...
            $sqlin="DELETE FROM  ref_tingkat_keparahan  WHERE kode=$id;";
            $rsl=$db->query($sqlin);
            if(isset($rsl->error) and $rsl->error===true){
                    $msg['success']=false;
                    $msg['message']="Error, ".$rsl->query_last_message;
	...
	public function validasiform() 
    {
		...
		if(trim($_POST['keterangan'])==''){
            $pesan["keterangan"]="Nama harus diisi!";   
