...
class Ref_biaya_perbaikan_Controller extends Admin_Template_Controller {
	...
	public function index() {
        ...
        $tpl  = new View("ref_biaya_perbaikan");
	...
	public function listdata() {
		...
		while($data = $db->fetchObject($list_qry))
		{
			...
			$ListData[$i]['nama_biaya_perbaikan']=$data->nama_biaya_perbaikan;
        ...
        global $dcistem;
           ...
           if(trim($proses)=="save")
           {
            //    echo json_encode("tes");  
               ...
               $nama_biaya_perbaikan	=trim($_POST['nama_biaya_perbaikan']);

               

               $validasi=$this->validasiform();   
               if(count($validasi['arrayerror'])==0){
                   $cek=$db->select("kode","ref_biaya_perbaikan")->where("kode='".$kode."'")->get(0);
                   if(empty($cek)){
						// $TglSkrg=date("Y-m-d H:i:s");
						$sqlin				="";
						$kode_val			=$master->scurevaluetable($kode);
						$nama_biaya_perbaikan_val	=$master->scurevaluetable($nama_biaya_perbaikan);

                       
                       $cols="kode,nama_biaya_perbaikan";
                       $values="$kode_val,$nama_biaya_perbaikan_val";
                       $sqlin="INSERT INTO ref_biaya_perbaikan ($cols) VALUES ($values);";
			...
			$tpl  = new View("form_ref_biaya_perbaikan");
	...
	public function edit($id,$proses=""){  
        ...
        {    
                    ...
                    if(count($validasi['arrayerror'])==0){
                        $cek=$db->select("kode","ref_biaya_perbaikan")->where("kode='".$kode."'")->get(0);
                   		if(empty($cek) || $kode == $id){
							...
							$cols_and_vals="kode=$kode_val,nama_biaya_perbaikan=$nama_biaya_perbaikan_val";
						
							$sqlin="UPDATE ref_biaya_perbaikan SET $cols_and_vals WHERE kode=$id;";
            ...
            $partner=new Ref_biaya_perbaikan_Model();

            $tpl  = new View("form_ref_biaya_perbaikan");
            $detail=$partner->getDetailRefBiayaPerbaikan($id);
            $tpl->detail = $detail;
    ...
    public function detail($id){     
		global $dcistem;
		$db   = $dcistem->getOption("framework/db");
		$tpl  = new View("detail_ref_biaya_perbaikan");
		$insidenMdl    = new Ref_biaya_perbaikan_Model();
		date_default_timezone_set("Asia/Jakarta");
		$detail=$insidenMdl->getDetailRefBiayaPerbaikan($id);
		// $tpl->list_contractor =Model::getOptionList("partner","id","name","name ASC","ifnull(is_contractor,0)=1"); 

		$tpl->kode            		= $detail->kode;
		$tpl->nama_biaya_perbaikan       	= $detail->nama_biaya_perbaikan;
	...
	public function del($id){     
        ...
        if(trim($id)=="" or  $id == null){
            ...
            $sqlin="DELETE FROM  ref_biaya_perbaikan  WHERE kode=$id;";
            $rsl=$db->query($sqlin);
            if(isset($rsl->error) and $rsl->error===true){
                    $msg['success']=false;
                    $msg['message']="Error, ".$rsl->query_last_message;
	...
	public function validasiform() 
    {
		...
		if(trim($_POST['nama_biaya_perbaikan'])==''){
            $pesan["nama_biaya_perbaikan"]="Nama harus diisi!";   
