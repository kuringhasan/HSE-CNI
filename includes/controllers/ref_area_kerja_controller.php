...
class Ref_area_kerja_Controller extends Admin_Template_Controller {
	...
	public function listdata() {
		...
		while($data = $db->fetchObject($list_qry))
		{
			...
			$ListData[$i]['nama_area']=$data->nama_area;
           ...
           if(trim($proses)=="save")
           {
            //    echo json_encode("tes");  
               ...
               if(count($validasi['arrayerror'])==0){
                   $cek=$db->select("kode","ref_area_kerja")->where("kode='".$kode."'")->get(0);
                   if(empty($cek)){
						// $TglSkrg=date("Y-m-d H:i:s");
						$sqlin				="";
						$kode_val			=$master->scurevaluetable($kode);
						$nama_area_val	=$master->scurevaluetable($nama_area);

                       
                       $cols="kode,nama_area";
                       $values="$kode_val,$nama_area_val";
                       $sqlin="INSERT INTO ref_area_kerja ($cols) VALUES ($values);";
			...
			$tpl  = new View("form_ref_area_kerja");
	...
	public function edit($id,$proses=""){  
        ...
        {    
                    ...
                    if(count($validasi['arrayerror'])==0){
                        $cek=$db->select("kode","ref_area_kerja")->where("kode='".$kode."'")->get(0);
                   		if(empty($cek) || $kode == $id){
							...
							$cols_and_vals="kode=$kode_val,nama_area=$nama_area_val";
						
							$sqlin="UPDATE ref_area_kerja SET $cols_and_vals WHERE kode=$id;";
            ...
            $partner=new Ref_area_kerja_Model();

            $tpl  = new View("form_ref_area_kerja");
            $detail=$partner->getDetailRefArea($id);
            $tpl->detail = $detail;
    ...
    public function detail($id){     
		global $dcistem;
		$db   = $dcistem->getOption("framework/db");
		$tpl  = new View("detail_ref_area_kerja");
		$insidenMdl    = new Ref_area_kerja_Model();
		date_default_timezone_set("Asia/Jakarta");
		$detail=$insidenMdl->getDetailRefArea($id);
		// $tpl->list_contractor =Model::getOptionList("partner","id","name","name ASC","ifnull(is_contractor,0)=1"); 

		$tpl->kode            		= $detail->kode;
		$tpl->nama_area       	= $detail->nama_area;
	...
	public function del($id){     
        ...
        if(trim($id)=="" or  $id == null){
            ...
            $sqlin="DELETE FROM  ref_area_kerja  WHERE kode=$id;";
            $rsl=$db->query($sqlin);
            if(isset($rsl->error) and $rsl->error===true){
                    $msg['success']=false;
                    $msg['message']="Error, ".$rsl->query_last_message;
	...
	public function validasiform() 
    {
		...
		if(trim($_POST['nama_area'])==''){
            $pesan["nama_area"]="Nama harus diisi!";   
