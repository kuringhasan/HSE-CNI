...
class Ref_Kondisi_Tidak_Standar_Controller extends Admin_Template_Controller {
	...
	public function index() {
        ...
        $tpl  = new View("ref_kondisi_tidak_standar");
	...
	public function listdata() {
		...
		while($data = $db->fetchObject($list_qry))
		{
			...
			$ListData[$i]['nama_kondisi']=$data->nama_kondisi;
           ...
           if(trim($proses)=="save")
           {
            //    echo json_encode("tes");  
               ...
               if(count($validasi['arrayerror'])==0){
                   $cek=$db->select("kode","ref_kondisi_tidak_standar")->where("kode='".$kode."'")->get(0);
                   if(empty($cek)){
						// $TglSkrg=date("Y-m-d H:i:s");
						$sqlin				="";
						$kode_val			=$master->scurevaluetable($kode);
						$nama_kondisi_val	=$master->scurevaluetable($nama_kondisi);

                       
                       $cols="kode,nama_kondisi";
                       $values="$kode_val,$nama_kondisi_val";
                       $sqlin="INSERT INTO ref_kondisi_tidak_standar ($cols) VALUES ($values);";
			...
			$tpl  = new View("form_ref_kondisi_tidak_standar");
	...
	public function edit($id,$proses=""){  
        ...
        {    
                    ...
                    if(count($validasi['arrayerror'])==0){
                        $cek=$db->select("kode","ref_kondisi_tidak_standar")->where("kode='".$kode."'")->get(0);
                   		if(empty($cek) || $kode == $id){
							...
							$cols_and_vals="kode=$kode_val,nama_kondisi=$nama_kondisi_val";
						
							$sqlin="UPDATE ref_kondisi_tidak_standar SET $cols_and_vals WHERE kode=$id;";
            ...
            $partner=new Ref_kondisi_tidak_standar_Model();

            $tpl  = new View("form_ref_kondisi_tidak_standar");
            $detail=$partner->getDetailRefKondisiTidakStandar($id);
            $tpl->detail = $detail;
    ...
    public function detail($id){     
		global $dcistem;
		$db   = $dcistem->getOption("framework/db");
		$tpl  = new View("detail_ref_kondisi_tidak_standar");
		$insidenMdl    = new Ref_kondisi_tidak_standar_Model();
		date_default_timezone_set("Asia/Jakarta");
		$detail=$insidenMdl->getDetailRefKondisiTidakStandar($id);
		// $tpl->list_contractor =Model::getOptionList("partner","id","name","name ASC","ifnull(is_contractor,0)=1"); 

		$tpl->kode            		= $detail->kode;
		$tpl->nama_kondisi       	= $detail->nama_kondisi;
	...
	public function del($id){     
        ...
        if(trim($id)=="" or  $id == null){
            ...
            $sqlin="DELETE FROM  ref_kondisi_tidak_standar  WHERE kode=$id;";
            $rsl=$db->query($sqlin);
            if(isset($rsl->error) and $rsl->error===true){
                    $msg['success']=false;
                    $msg['message']="Error, ".$rsl->query_last_message;
	...
	public function validasiform() 
    {
		...
		if(trim($_POST['nama_kondisi'])==''){
            $pesan["nama_kondisi"]="Nama harus diisi!";   
