...
class Ref_Lokasikerja_Controller extends Admin_Template_Controller {
    ...
    public function listdata() {
       ...
       $draw=$_REQUEST['draw'];
        ...
        $list_qry=$db->select("SQL_CALC_FOUND_ROWS id,name,alamat,zona_waktu,koordinat,radius_absen,date_format(check_in,'%H:%i') check_in,date_format(check_out,'%H:%i') check_out ","lokasi_kerja")
        ...
        while($data = $db->fetchObject($list_qry))
        {
            ...
            $ListData[$i]['koordinat']=$data->koordinat;
            $ListData[$i]['zona_waktu']=$data->zona_waktu;
            $ListData[$i]['radius_absen']=$data->radius_absen;
            $ListData[$i]['masuk']=$data->check_in;
            $ListData[$i]['pulang']=$data->check_out;
...
public function edit($id,$proses=""){     
	...
	{    
	    ...
	    echo json_encode($msg);   
	       ...
	       $tpl  = new View("form_pit");
           $obat = new Ref_Obat_Model();
    	   $list_contractor=Model::getOptionList("partner","id","name","name ASC","ifnull(is_contractor,0)=1"); 
	    
        $tpl->list_contractor =$list_contractor;
            $detail=$obat->getObat($id);
			$tpl->detail = $detail;
	    	$tpl->url_add = url::current("add");
	    	$tpl->url_jsonData		= url::current("jsonData");
        	$tpl->url_comboAjax		=url::current("comboAjax");
