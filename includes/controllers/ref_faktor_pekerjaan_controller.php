...
class Ref_faktor_pekerjaan_Controller extends Admin_Template_Controller {
	...
	public function index() {
        ...
        $tpl  = new View("ref_faktor_pekerjaan");
	...
	public function listdata() {
		...
		while($data = $db->fetchObject($list_qry))
		{
			...
			$ListData[$i]['nama_faktor_pekerjaan']=$data->nama_faktor_pekerjaan;
        ...
        global $dcistem;
           ...
           if(trim($proses)=="save")
           {
            //    echo json_encode("tes");  
               ...
               $nama_faktor_pekerjaan	=trim($_POST['nama_faktor_pekerjaan']);

               

               $validasi=$this->validasiform();   
               if(count($validasi['arrayerror'])==0){
                   $cek=$db->select("kode","ref_faktor_pekerjaan")->where("kode='".$kode."'")->get(0);
                   if(empty($cek)){
						// $TglSkrg=date("Y-m-d H:i:s");
						$sqlin				="";
						$kode_val			=$master->scurevaluetable($kode);
						$nama_faktor_pekerjaan_val	=$master->scurevaluetable($nama_faktor_pekerjaan);

                       
                       $cols="kode,nama_faktor_pekerjaan";
                       $values="$kode_val,$nama_faktor_pekerjaan_val";
                       $sqlin="INSERT INTO ref_faktor_pekerjaan ($cols) VALUES ($values);";
			...
			$tpl  = new View("form_ref_faktor_pekerjaan");
	...
	public function edit($id,$proses=""){  
        ...
        {    
                    ...
                    if(count($validasi['arrayerror'])==0){
                        $cek=$db->select("kode","ref_faktor_pekerjaan")->where("kode='".$kode."'")->get(0);
                   		if(empty($cek) || $kode == $id){
							...
							$cols_and_vals="kode=$kode_val,nama_faktor_pekerjaan=$nama_faktor_pekerjaan_val";
						
							$sqlin="UPDATE ref_faktor_pekerjaan SET $cols_and_vals WHERE kode=$id;";
            ...
            $partner=new Ref_faktor_pekerjaan_Model();

            $tpl  = new View("form_ref_faktor_pekerjaan");
            $detail=$partner->getDetailRefFaktorPekerjaan($id);
            $tpl->detail = $detail;
    ...
    public function detail($id){     
		global $dcistem;
		$db   = $dcistem->getOption("framework/db");
		$tpl  = new View("detail_ref_faktor_pekerjaan");
		$insidenMdl    = new Ref_faktor_pekerjaan_Model();
		date_default_timezone_set("Asia/Jakarta");
		$detail=$insidenMdl->getDetailRefFaktorPekerjaan($id);
		// $tpl->list_contractor =Model::getOptionList("partner","id","name","name ASC","ifnull(is_contractor,0)=1"); 

		$tpl->kode            		= $detail->kode;
		$tpl->nama_faktor_pekerjaan       	= $detail->nama_faktor_pekerjaan;
	...
	public function del($id){     
        ...
        if(trim($id)=="" or  $id == null){
            ...
            $sqlin="DELETE FROM  ref_faktor_pekerjaan  WHERE kode=$id;";
            $rsl=$db->query($sqlin);
            if(isset($rsl->error) and $rsl->error===true){
                    $msg['success']=false;
                    $msg['message']="Error, ".$rsl->query_last_message;
	...
	public function validasiform() 
    {
		...
		if(trim($_POST['nama_faktor_pekerjaan'])==''){
            $pesan["nama_faktor_pekerjaan"]="Nama harus diisi!";   
