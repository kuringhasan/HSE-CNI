...
class Ref_Jenis_Kecelakaan_Kerja_Controller extends Admin_Template_Controller {
	...
	public function index() {
        ...
        $tpl  = new View("ref_jenis_kecelakaan_kerja");
	...
	public function listdata() {
		...
		while($data = $db->fetchObject($list_qry))
		{
			...
			$ListData[$i]['nama_kecelakaan']=$data->nama_kecelakaan;
        ...
        global $dcistem;
           ...
           if(trim($proses)=="save")
           {
            //    echo json_encode("tes");  
               ...
               $nama_kecelakaan	=trim($_POST['nama_kecelakaan']);

               

               $validasi=$this->validasiform();   
               if(count($validasi['arrayerror'])==0){
                   $cek=$db->select("kode","ref_jenis_kecelakaan_kerja")->where("kode='".$kode."'")->get(0);
                   if(empty($cek)){
						// $TglSkrg=date("Y-m-d H:i:s");
						$sqlin				="";
						$kode_val			=$master->scurevaluetable($kode);
						$nama_kecelakaan_val	=$master->scurevaluetable($nama_kecelakaan);

                       
                       $cols="kode,nama_kecelakaan";
                       $values="$kode_val,$nama_kecelakaan_val";
                       $sqlin="INSERT INTO ref_jenis_kecelakaan_kerja ($cols) VALUES ($values);";
			...
			$tpl  = new View("form_ref_jenis_kecelakaan_kerja");
	...
	public function edit($id,$proses=""){  
        ...
        {    
                    ...
                    if(count($validasi['arrayerror'])==0){
                        $cek=$db->select("kode","ref_jenis_kecelakaan_kerja")->where("kode='".$kode."'")->get(0);
                   		if(empty($cek) || $kode == $id){
							...
							$cols_and_vals="kode=$kode_val,nama_kecelakaan=$nama_kecelakaan_val";
						
							$sqlin="UPDATE ref_jenis_kecelakaan_kerja SET $cols_and_vals WHERE kode=$id;";
            ...
            $partner=new Ref_jenis_kecelakaan_kerja_Model();
    ...
    public function detail($id){     
		global $dcistem;
		$db   = $dcistem->getOption("framework/db");
		$tpl  = new View("detail_ref_jenis_kecelakaan_kerja");
		$insidenMdl    = new Ref_jenis_kecelakaan_kerja_Model();
		date_default_timezone_set("Asia/Jakarta");
		...
		$tpl->nama_kecelakaan       	= $detail->nama_kecelakaan;
		

		// var_dump($tpl);

		$this->tpl->content_title = "Detail Ref jenis_kecelakaan_kerja";
		$tpl->detail = $tpl;
		$tpl->render();   
 	} 
	...
	public function del($id){     
        ...
        if(trim($id)=="" or  $id == null){
            ...
            $sqlin="DELETE FROM  ref_jenis_kecelakaan_kerja  WHERE kode=$id;";
            $rsl=$db->query($sqlin);
            if(isset($rsl->error) and $rsl->error===true){
                    $msg['success']=false;
                    $msg['message']="Error, ".$rsl->query_last_message;
	...
	public function validasiform() 
    {
		...
		if(trim($_POST['nama_kecelakaan'])==''){
            $pesan["nama_kecelakaan"]="Nama Jenis Kecelakaan harus diisi!";   
            $msg[]="Nama Jenis Kecelakaan harus diisi!";
        }
		// if(trim($_POST['nik'])==''){
