...
class Reports_Daily_Controller extends Admin_Template_Controller {
    ...
    public function listdata() {
       ...
       // $search=$admin->SearchDependingLevel("events","kps.petugas");
        ...
        if( trim($status)<>"" ){   //name
            $keriteria[]="status_bunting='".$status."'";
        }
		...
		*/
        ...
        // if(trim($tahun)<>"" and trim($bulan)<>""){
            ...
            pro.week,pro.partner_id,p.name,p.alias,qty,ifnull(verification,0) verification,verification_date,verifikator","production pro
            ...
            while($data = $db->fetchObject($list_qry))
            {
                ...
                $ListData[$i]['Qty']=number_format($data->qty,2,",",".");
               ...
               // $tombol=$tombol."&nbsp".$login->privilegeInputForm("link","","","<i class=\"fa fa-file-text-o\"></i>",$this->page->PageID,"detail","title='Detail Mutasi' href=\"".$url_detail."\" class=\"btn btn-primary btn-xs btn-detail-data\" target=\"_blank\"  role=\"".$data->id."\" data-toggle=\"modal\" data-remote=\"false\" data-target=\"#largeModal\"");
 ...
 public function add($proses=""){     
	...
	{    
	    ...
	    echo json_encode($msg);   
		    ...
		    $ip=core::get_ip();
	    	$tpl  = new View("form_production");
            $list_kontraktor=Model::getOptionList("partner","id","name","","ifnull(is_company,0)=1"); 
 ...
 public function detail($id){     
     global $dcistem;
        ...
        $this->tpl->content_title = "Detail Data Sapi";
        $tpl->content = $tpl;
        $tpl->render();   
  } 
