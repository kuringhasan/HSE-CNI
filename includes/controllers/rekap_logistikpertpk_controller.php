...
class Rekap_Logistikpertpk_Controller extends Admin_Template_Controller {
	...
	public function index() {
	   global $dcistem;
		...
		$tpl  = new View("rekap_logistikpertpk");
    ...
    public function listdata($action="periode") {
 	    global $dcistem;
       ...
       $db   = $dcistem->getOption("framework/db");
        ...
        if(trim($action)=="periode"){
           ...
           $list_qry2=$db->select("logt.id,logt.transaction_id,logt.periode_id,pe.start_date,logt.anggota_id,C_ANGGOTA no_anggota,ang.NAMA nama_anggota,
                kel.mcp_id,kel.name nama_kelompok,trx_date,date_format(trx_date,'%d/%m/%Y') as tanggal_transaksi,created,operator,pengambil,total,
                pegawai_id,barang_id,brg.name nama_barang,log.harga,jumlah_package,jumlah,sub_total","logistik_trx logt
                inner join logistik log on log.trx_id=logt.id
                inner join barang brg on brg.id=log.barang_id
                inner join periode pe on pe.id=logt.periode_id
                inner join anggota ang on ang.ID_ANGGOTA=logt.anggota_id
                inner join kelompok kel on kel.id=ang.ID_KELOMPOK")
    		->where($filter)->orderby("logt.periode_id desc,kel.mcp_id asc,ang.ID_KELOMPOK asc,ang.ID_KELOMPOK_HARGA asc,anggota_id asc, trx_date asc, barang_id asc")->lim($start,$length);//
            ...
            while($data = $db->fetchObject($list_qry2))
            {
                ...
                $ListRekap[$i]['No']=$no;
                $ListRekap[$i]['tpk']=$ref_tpk[$data->mcp_id];
                 $ListRekap[$i]['anggota']=$data->no_anggota." - ".$data->nama_anggota;
                ...
                $ListRekap[$i]['brg_name']=$data->nama_barang;
                $ListRekap[$i]['harga_satuan']=number_format($data->harga,0,",",".");
                $ListRekap[$i]['jml_package']=$data->jumlah_package;
                $ListRekap[$i]['jml_qty']=number_format($data->jumlah,0,",",".");                
                $ListRekap[$i]['jml_harga']=number_format($data->sub_total,0,",",".");
