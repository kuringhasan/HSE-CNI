...
class Monthly_Expitore_Controller extends Admin_Template_Controller {
	...
	public function index() {
	   global $dcistem;
        ...
        $tpl->content_title		= "Recap Monthly EX-PIT Ore";
        $this->tpl->content_title = $tpl->content_title;
    ...
    public function listdata() {
       ...
       // echo "<pre>";print_r($search);echo "</pre>";
        ...
        if( trim($bulan)<>"" ){   //name
                ...
                if( trim($tahun)<>"" ){
                    ...
                    $keriteria[]="DATE_FORMAT(month,'%Y-%m')='".$tahun."-".$bulan."'";
		...
		*/
        ...
        // if(trim($tahun)<>"" and trim($bulan)<>""){
            ...
            $list_qry=$db->select("mto.id,mto.contractor_id,p.name,p.alias,month,right(month,2) bulan,left(month,4) tahun,ritase,
            target,cumulative_target,quantity,cumulative_quantity,
            locked","recap_monthly_transit_ore mto
            inner join partner p on p.id=mto.contractor_id")
            ...
            while($data = $db->fetchObject($list_qry))
            {
                ...
                $nama_bulan = $master->namabulanIN((int)$data->bulan);              
                $ListData[$i]['bulan']=$nama_bulan." ".$data->tahun;
                $ListData[$i]['ritase']=$data->ritase;
                $ListData[$i]['target']=$data->target;
                
                $ListData[$i]['cumulative_target']=$data->cumulative_target;
                $ListData[$i]['quantity']=$data->quantity;
                $ListData[$i]['cumulative_quantity']=$data->cumulative_quantity;
