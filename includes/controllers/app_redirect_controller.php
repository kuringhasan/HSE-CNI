...
class App_Redirect_Controller extends Admin_Template_Controller {
    ...
    public function listdata() {
       ...
       $draw=$_REQUEST['draw'];
        ...
        $cols=array(0=>"AppWebRedirectLevelID",
                    1=>"AppWebRedirectTargetID");
        $order= $cols[$_REQUEST['order'][0]['column']]." ".$_REQUEST['order'][0]['dir'];
            
        $list_qry=$db->select("AppWebRedirectPageID ID,AppWebRedirectLevelID LevelID, AppLevelListLevelName LevelName,
        AppWebRedirectTargetID PageTargetID,pg.AppPageListPageTitle TargetPageName,AppPageListPageController TargetController","tbaappwebredirect red
        inner join tbaapplevellist lv on lv.AppLevelListLevelID=red.AppWebRedirectLevelID
        inner join tbaapppagelist pg on pg.AppPageListPageID=red.AppWebRedirectTargetID")
        ...
        while($data = $db->fetchObject($list_qry))
        {
            ...
            $ListData[$i]['ID']=$data->ID;
             $ListData[$i]['LevelID']=$data->LevelID;
            $ListData[$i]['LevelName']=$data->LevelName;
            $ListData[$i]['TargetPageID']=$data->PageTargetID;
            $ListData[$i]['TargetPageName']=$data->TargetPageName."<br />".$data->TargetController;
        ...
        $jml=$db->select("count(AppWebRedirectPageID) as jml_data","tbaappwebredirect")->get(0);
  ...
  public function add($proses=""){     
     global $dcistem;
        ...
        if(trim($proses)=="save")
		{
            ...
            $qty_package	=trim($_POST['qty_package']);
	        ...
	        if(count($validasi['arrayerror'])==0){
		        ...
		        $barang_val	=$master->scurevaluetable($barang,"number");
                $satuan_val	=$master->scurevaluetable($satuan,"number");
		        $qty_package_val	=$master->scurevaluetable($qty_package,"number");
		        //$hide_val	=$master->scurevaluetable($hide,"string");
		        
				$cols="name,barang_id,satuan_id,qty_package";
				$values="$nama_val,$barang_val,$satuan_val,$qty_package_val";
				$sqlin="INSERT INTO barang_package ($cols) VALUES ($values);";
	    	...
	    	$tpl  = new View("form_package");
    	    
            $tpl->list_barang =Model::getOptionList("barang","id","name","name ASC"); 
			$tpl->list_satuan =Model::getOptionList("units_odoo","id","name","id ASC"); 
	...
	{    
	    ...
	    { 
	        ...
	        if(count($validasi['arrayerror'])==0){
		        ...
		        $qty_package_val	=$master->scurevaluetable($qty_package,"number");
               	...
               	$cols_and_vals="name=$nama_val,barang_id=$barang_val,satuan_id=$satuan_val,qty_package=$qty_package_val";
				
				$sqlin="UPDATE barang_package SET $cols_and_vals WHERE id=$id;";
	    ...
	    echo json_encode($msg);   
	      ...
	      $tpl  = new View("form_package");
            ...
            $tpl->list_barang =Model::getOptionList("barang","id","name","name ASC"); 
			$tpl->list_satuan =Model::getOptionList("units_odoo","id","name","id ASC"); 
            $detail=$db->select("pack.id,pack.name,barang_id,brg.name nama_barang,satuan_id,unt.name nama_satuan,
            qty_package","barang_package pack
            inner join barang brg on brg.id=pack.barang_id
            inner join units_odoo unt on unt.id=pack.satuan_id")->where("pack.id=$id")->get(0);
			$tpl->detail = $detail;
	    	$tpl->url_add = url::current("add");
	    	$tpl->url_jsonData		= url::current("jsonData");
        	$tpl->url_comboAjax		=url::current("comboAjax");
...
public function del($id){     
     global $dcistem;
        ...
        {
	        $Nama=$_POST['nama'];
	        $sqlin="DELETE FROM   barang_package  WHERE id=$id;";
	        $rsl=$db->query($sqlin);
			if(isset($rsl->error) and $rsl->error===true){
		   	 		$msg['success']=false;
                	$msg['message']="Error, ".$rsl->query_last_message;
  ...
  public function validasiform($aksi="add",$kode_lama="") 
    {
        ...
        $msg=array();
      	$pesan=array();
    	if(trim($_POST['name'])==''){
            $pesan["name"]="Nama obat harus diisi!";   
            $msg[]="Nama obat harus diisi!";
        }
