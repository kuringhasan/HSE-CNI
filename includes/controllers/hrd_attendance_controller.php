...
class Hrd_Attendance_Controller extends Admin_Template_Controller {
    ...
    public function listdata() {
       ...
       // $search=$admin->SearchDependingLevel("events","kps.petugas");
        ...
        $kategori     = $requestData['columns'][4]['search']['value'];
        ...
        if( trim($kategori)<>"" ){   //name
            $keriteria[]="category='".$kategori."'";
        }
		...
		*/
        ...
        // if(trim($tahun)<>"" and trim($bulan)<>""){
            ...
            $list_qry=$db->select("SQL_CALC_FOUND_ROWS att.id,transaction_id,em.no_induk,att.partner_id,p.name,p.alias,gelar_depan,gelar_belakang,
            DATE_FORMAT(tanggal,'%d/%m/%Y') tanggal, DATE_FORMAT(att.check_in,'%d/%m/%Y %H:%i:%s') check_in,DATE_FORMAT(att.check_out,'%d/%m/%Y %H:%i:%s') check_out,kehadiran,
            category,att.zona_waktu,received_time,shift,lokasi_id,lk.name lokasi_name","attendance att
            inner join partner p on p.id=att.partner_id
            inner join employees em on em.partner_id=p.id
            inner join lokasi_kerja lk on lk.id=att.lokasi_id ")
            ...
            while($data = $db->fetchObject($list_qry))
            {
                ...
                $ListData[$i]['Checkout']=$data->check_out;
                $ListData[$i]['Kehadiran']=$data->kehadiran;
                $ListData[$i]['zona_waktu']=$data->zona_waktu;
                $ListData[$i]['received_time']=$data->received_time;
                $ListData[$i]['total_quantity']=$data->total_quantity;
                $ListData[$i]['Lokasi']=$data->lokasi_name;
                $ListData[$i]['Category']=$data->category=="R"?"Reguler":"Lembur";
