...
class App_Level_Controller extends Admin_Template_Controller {
    ...
    public function listdata() {
       ...
       $draw=$_REQUEST['draw'];
        ...
        $cols=array(0=>"AppLevelListLevelID",
                    1=>"AppLevelListLevelName");
        $order= $cols[$_REQUEST['order'][0]['column']]." ".$_REQUEST['order'][0]['dir'];
            
        $list_qry=$db->select("AppLevelListLevelID,AppLevelListLevelName,AppLevelListRefName,AppLevelUnit,
        AppLevelUnitName","tbaapplevellist")
        ...
        {
            $ListData[$i]['No']=$no;
            $ListData[$i]['ID']=$data->AppLevelListLevelID;
            $ListData[$i]['Nama']=$data->AppLevelListLevelName;
            $ListData[$i]['RefName']=$data->AppLevelListRefName;
            $ListData[$i]['UnitID']=$data->AppLevelUnit;
            $ListData[$i]['UnitName']=$data->AppLevelUnitName;
        ...
        $jml=$db->select("count(AppLevelListLevelID) as jml_data","tbaapplevellist")->get(0);
        $jml_filtered=$db->select("count(AppLevelListLevelID) as jml_data","tbaapplevellist")->where($filter)->get(0);
...
public function edit($id,$proses=""){     
	...
	{    
	    ...
	    { 
	        ...
	        if(count($validasi['arrayerror'])==0){
		        ...
		        $aktif_val	=$master->scurevaluetable($aktif,"number");
               	...
               	$cols_and_vals="name=$nama_val,category=$category_val,active=$aktif_val";
				$sqlin="UPDATE keswan_obat SET $cols_and_vals WHERE id=$id;";
	    ...
	    echo json_encode($msg);   
	       ...
	       $tpl  = new View("form_obat");
           $pengobatan = new List_Pengobatan_Model();
    	    $list_category=Model::getOptionList("keswan_obat_category","code","case when ifnull(category,'')='' 
            then code else concat(code,'-',category) end  category","category ASC"); 
    	    
            $tpl->list_category =$list_category;
            $detail=$pengobatan->getRefObat($id);
			$tpl->detail = $detail;
	    	$tpl->url_add = url::current("add");
	    	$tpl->url_jsonData		= url::current("jsonData");
        	$tpl->url_comboAjax		=url::current("comboAjax");
