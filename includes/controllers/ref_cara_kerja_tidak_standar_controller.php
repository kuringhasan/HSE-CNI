...
class Ref_Cara_Kerja_Tidak_Standar_Controller extends Admin_Template_Controller {
	...
	public function index() {
        ...
        $tpl  = new View("ref_cara_kerja_tidak_standar");
	...
	public function listdata() {
		...
		","ref_cara_kerja_tidak_standar a")
		->where($filter)
		->orderBy($order)
		->lim($start,$length)
		;
		...
		while($data = $db->fetchObject($list_qry))
		{
			...
			$ListData[$i]['nama_cara_kerja']=$data->nama_cara_kerja;
        ...
        global $dcistem;
           ...
           if(trim($proses)=="save")
           {
            //    echo json_encode("tes");  
               ...
               $nama_cara_kerja	=trim($_POST['nama_cara_kerja']);

               

               $validasi=$this->validasiform();   
               if(count($validasi['arrayerror'])==0){
                   $cek=$db->select("kode","ref_cara_kerja_tidak_standar")->where("kode='".$kode."'")->get(0);
                   if(empty($cek)){
						// $TglSkrg=date("Y-m-d H:i:s");
						$sqlin				="";
						$kode_val			=$master->scurevaluetable($kode);
						$nama_cara_kerja_val	=$master->scurevaluetable($nama_cara_kerja);

                       
                       $cols="kode,nama_cara_kerja";
                       $values="$kode_val,$nama_cara_kerja_val";
                       $sqlin="INSERT INTO ref_cara_kerja_tidak_standar ($cols) VALUES ($values);";
			...
			$tpl  = new View("form_ref_cara_kerja_tidak_standar");
	...
	public function edit($id,$proses=""){  
        ...
        {    
                    ...
                    if(count($validasi['arrayerror'])==0){
                        $cek=$db->select("kode","ref_cara_kerja_tidak_standar")->where("kode='".$kode."'")->get(0);
                   		if(empty($cek) || $kode == $id){
							...
							$cols_and_vals="kode=$kode_val,nama_cara_kerja=$nama_cara_kerja_val";
						
							$sqlin="UPDATE ref_cara_kerja_tidak_standar SET $cols_and_vals WHERE kode=$id;";
            ...
            $partner=new Ref_Cara_Kerja_Tidak_Standar_Model();

            $tpl  = new View("form_ref_cara_kerja_tidak_standar");
            $detail=$partner->getDetailCaraKerjaTidakStandar($id);
            $tpl->detail = $detail;
    ...
    public function detail($id){     
		global $dcistem;
		$db   = $dcistem->getOption("framework/db");
		$tpl  = new View("detail_ref_cara_kerja_tidak_standar");
		$insidenMdl    = new Ref_Cara_Kerja_Tidak_Standar_Model();
		date_default_timezone_set("Asia/Jakarta");
		$detail=$insidenMdl->getDetailCaraKerjaTidakStandar($id);
		// $tpl->list_contractor =Model::getOptionList("partner","id","name","name ASC","ifnull(is_contractor,0)=1"); 

		$tpl->kode            		= $detail->kode;
		$tpl->nama_cara_kerja       	= $detail->nama_cara_kerja;
	...
	public function del($id){     
        ...
        if(trim($id)=="" or  $id == null){
            ...
            $sqlin="DELETE FROM  ref_cara_kerja_tidak_standar  WHERE kode=$id;";
            $rsl=$db->query($sqlin);
            if(isset($rsl->error) and $rsl->error===true){
                    $msg['success']=false;
                    $msg['message']="Error, ".$rsl->query_last_message;
	...
	public function validasiform() 
    {
		...
		if(trim($_POST['nama_cara_kerja'])==''){
            $pesan["nama_cara_kerja"]="Nama harus diisi!";   
