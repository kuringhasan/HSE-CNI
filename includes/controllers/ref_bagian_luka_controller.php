...
class Ref_bagian_luka_Controller extends Admin_Template_Controller {
	...
	public function listdata() {
		...
		while($data = $db->fetchObject($list_qry))
		{
			...
			$ListData[$i]['nama_bagian_luka']=$data->nama_bagian_luka;
        ...
        global $dcistem;
           ...
           if(trim($proses)=="save")
           {
            //    echo json_encode("tes");  
               ...
               $nama_bagian_luka	=trim($_POST['nama_bagian_luka']);

               

               $validasi=$this->validasiform();   
               if(count($validasi['arrayerror'])==0){
                   $cek=$db->select("kode","ref_bagian_luka")->where("kode='".$kode."'")->get(0);
                   if(empty($cek)){
						// $TglSkrg=date("Y-m-d H:i:s");
						$sqlin				="";
						$kode_val			=$master->scurevaluetable($kode);
						$nama_bagian_luka_val	=$master->scurevaluetable($nama_bagian_luka);

                       
                       $cols="kode,nama_bagian_luka";
                       $values="$kode_val,$nama_bagian_luka_val";
                       $sqlin="INSERT INTO ref_bagian_luka ($cols) VALUES ($values);";
			...
			$tpl  = new View("form_ref_bagian_luka");
	...
	public function edit($id,$proses=""){  
        ...
        {    
                    ...
                    if(count($validasi['arrayerror'])==0){
                        $cek=$db->select("kode","ref_bagian_luka")->where("kode='".$kode."'")->get(0);
                   		if(empty($cek) || $kode == $id){
							...
							$cols_and_vals="kode=$kode_val,nama_bagian_luka=$nama_bagian_luka_val";
						
							$sqlin="UPDATE ref_bagian_luka SET $cols_and_vals WHERE kode=$id;";
            ...
            $partner=new Ref_bagian_luka_Model();

            $tpl  = new View("form_ref_bagian_luka");
            $detail=$partner->getDetailRefBagianLuka($id);
            $tpl->detail = $detail;
    ...
    public function detail($id){     
		global $dcistem;
		$db   = $dcistem->getOption("framework/db");
		$tpl  = new View("detail_ref_bagian_luka");
		$insidenMdl    = new Ref_bagian_luka_Model();
		date_default_timezone_set("Asia/Jakarta");
		$detail=$insidenMdl->getDetailRefBagianLuka($id);
		// $tpl->list_contractor =Model::getOptionList("partner","id","name","name ASC","ifnull(is_contractor,0)=1"); 

		$tpl->kode            		= $detail->kode;
		$tpl->nama_bagian_luka       	= $detail->nama_bagian_luka;
	...
	public function del($id){     
        ...
        if(trim($id)=="" or  $id == null){
            ...
            $sqlin="DELETE FROM  ref_bagian_luka  WHERE kode=$id;";
            $rsl=$db->query($sqlin);
            if(isset($rsl->error) and $rsl->error===true){
                    $msg['success']=false;
                    $msg['message']="Error, ".$rsl->query_last_message;
	...
	public function validasiform() 
    {
		...
		if(trim($_POST['nama_bagian_luka'])==''){
            $pesan["nama_bagian_luka"]="Nama harus diisi!";   
