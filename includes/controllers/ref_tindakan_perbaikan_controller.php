...
class Ref_tindakan_perbaikan_Controller extends Admin_Template_Controller {
	...
	public function index() {
        ...
        $tpl  = new View("ref_tindakan_perbaikan");
	...
	public function listdata() {
		...
		while($data = $db->fetchObject($list_qry))
		{
			...
			$ListData[$i]['nama_tindakan']=$data->nama_tindakan;
           ...
           if(trim($proses)=="save")
           {
            //    echo json_encode("tes");  
               ...
               if(count($validasi['arrayerror'])==0){
                   $cek=$db->select("kode","ref_tindakan_perbaikan")->where("kode='".$kode."'")->get(0);
                   if(empty($cek)){
						// $TglSkrg=date("Y-m-d H:i:s");
						$sqlin				="";
						$kode_val			=$master->scurevaluetable($kode);
						$nama_tindakan_val	=$master->scurevaluetable($nama_tindakan);

                       
                       $cols="kode,nama_tindakan";
                       $values="$kode_val,$nama_tindakan_val";
                       $sqlin="INSERT INTO ref_tindakan_perbaikan ($cols) VALUES ($values);";
			...
			$tpl  = new View("form_ref_tindakan_perbaikan");
	...
	public function edit($id,$proses=""){  
        ...
        {    
                    ...
                    if(count($validasi['arrayerror'])==0){
                        $cek=$db->select("kode","ref_tindakan_perbaikan")->where("kode='".$kode."'")->get(0);
                   		if(empty($cek) || $kode == $id){
							...
							$cols_and_vals="kode=$kode_val,nama_tindakan=$nama_tindakan_val";
						
							$sqlin="UPDATE ref_tindakan_perbaikan SET $cols_and_vals WHERE kode=$id;";
            ...
            $partner=new Ref_tindakan_perbaikan_Model();

            $tpl  = new View("form_ref_tindakan_perbaikan");
            $detail=$partner->getDetailRefTindakanPerbaikan($id);
            $tpl->detail = $detail;
    ...
    public function detail($id){     
		global $dcistem;
		$db   = $dcistem->getOption("framework/db");
		$tpl  = new View("detail_ref_tindakan_perbaikan");
		$insidenMdl    = new Ref_tindakan_perbaikan_Model();
		date_default_timezone_set("Asia/Jakarta");
		$detail=$insidenMdl->getDetailRefTindakanPerbaikan($id);
		// $tpl->list_contractor =Model::getOptionList("partner","id","name","name ASC","ifnull(is_contractor,0)=1"); 

		$tpl->kode            		= $detail->kode;
		$tpl->nama_tindakan       	= $detail->nama_tindakan;
	...
	public function del($id){     
        ...
        if(trim($id)=="" or  $id == null){
            ...
            $sqlin="DELETE FROM  ref_tindakan_perbaikan  WHERE kode=$id;";
            $rsl=$db->query($sqlin);
            if(isset($rsl->error) and $rsl->error===true){
                    $msg['success']=false;
                    $msg['message']="Error, ".$rsl->query_last_message;
	...
	public function validasiform() 
    {
		...
		if(trim($_POST['nama_tindakan'])==''){
            $pesan["nama_tindakan"]="Nama harus diisi!";   
