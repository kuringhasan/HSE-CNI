...
class Ref_shift_Controller extends Admin_Template_Controller {
	...
	public function listdata() {
		...
		while($data = $db->fetchObject($list_qry))
		{
			...
			$ListData[$i]['nama_shift']=$data->nama_shift;
           ...
           if(trim($proses)=="save")
           {
            //    echo json_encode("tes");  
               ...
               if(count($validasi['arrayerror'])==0){
                   $cek=$db->select("kode","ref_shift")->where("kode='".$kode."'")->get(0);
                   if(empty($cek)){
						// $TglSkrg=date("Y-m-d H:i:s");
						$sqlin				="";
						$kode_val			=$master->scurevaluetable($kode);
						$nama_shift_val	=$master->scurevaluetable($nama_shift);

                       
                       $cols="kode,nama_shift";
                       $values="$kode_val,$nama_shift_val";
                       $sqlin="INSERT INTO ref_shift ($cols) VALUES ($values);";
	...
	public function edit($id,$proses=""){  
        ...
        {    
                    ...
                    if(count($validasi['arrayerror'])==0){
                        $cek=$db->select("kode","ref_shift")->where("kode='".$kode."'")->get(0);
                   		if(empty($cek) || $kode == $id){
							...
							$cols_and_vals="kode=$kode_val,nama_shift=$nama_shift_val";
						
							$sqlin="UPDATE ref_shift SET $cols_and_vals WHERE kode=$id;";
            ...
            $partner=new Ref_shift_Model();

            $tpl  = new View("form_ref_shift");
            $detail=$partner->getDetailRefshift($id);
            $tpl->detail = $detail;
    ...
    public function detail($id){     
		global $dcistem;
		$db   = $dcistem->getOption("framework/db");
		$tpl  = new View("detail_ref_shift");
		$insidenMdl    = new Ref_shift_Model();
		date_default_timezone_set("Asia/Jakarta");
		$detail=$insidenMdl->getDetailRefshift($id);
		// $tpl->list_contractor =Model::getOptionList("partner","id","name","name ASC","ifnull(is_contractor,0)=1"); 

		$tpl->kode            		= $detail->kode;
		$tpl->nama_shift       	= $detail->nama_shift;
	...
	public function del($id){     
        ...
        if(trim($id)=="" or  $id == null){
            ...
            $sqlin="DELETE FROM  ref_shift  WHERE kode=$id;";
            $rsl=$db->query($sqlin);
            if(isset($rsl->error) and $rsl->error===true){
                    $msg['success']=false;
                    $msg['message']="Error, ".$rsl->query_last_message;
	...
	public function validasiform() 
    {
		...
		if(trim($_POST['nama_shift'])==''){
            $pesan["nama_shift"]="Nama harus diisi!";   
