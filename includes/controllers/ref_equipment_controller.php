...
class Ref_Equipment_Controller extends Admin_Template_Controller {
    ...
    public function listdata() {
       ...
       $draw=$_REQUEST['draw'];
        ...
        $list_qry=$db->select("SQL_CALC_FOUND_ROWS eq.id,eq.category category_code, eqc.category category_name,eq.name,type,load_factor,merk,nomor,remarks,
vendor_id,p.name vendor_name,depricated","equipment eq 
        ...
        while($data = $db->fetchObject($list_qry))
        {
            ...
            $ListData[$i]['depricated']=$data->depricated=="1"?"<i class=\"fa fa-check\"></i>":"";
  ...
  public function add($proses=""){     
     global $dcistem;
        ...
        if(trim($proses)=="save")
		{
            ...
            $load_factor	=trim($_POST['load_factor']);
	        $depricated	=isset($_POST['depricated'])?"1":"" ;
	        ...
	        if(count($validasi['arrayerror'])==0){
                ...
                if(empty($cek)){
                    ...
                    $nomor_val	=$master->scurevaluetable($nomor);
                    $nama_val	=$master->scurevaluetable($nama);
                    $merk_val	=$master->scurevaluetable($merk);
                    $type_val	=$master->scurevaluetable($type);
                    $remarks_val	=$master->scurevaluetable($remarks);
                    $load_factor_val	=$master->scurevaluetable($load_factor,"number");
    		        $vendor_id_val	=$master->scurevaluetable($vendor_id,"number");
    		        $depricated_val	=$master->scurevaluetable($depricated);
    		        
    				$cols="category,name,type,load_factor,merk,nomor,remarks,vendor_id,depricated";
    				$values="$category_val,$nama_val,$type_val,$load_factor_val,$merk_val,$nomor_val,$remarks_val,$vendor_id_val,$depricated_val";
    				$sqlin="INSERT INTO equipment ($cols) VALUES ($values);";
	...
	{    
	    ...
	    if(trim($id)=="" or  $id == null){
	        ...
	        if(count($validasi['arrayerror'])==0){
            	...
            	$cols_and_vals="category=$category_val,name=$nama_val,type=$type_val,load_factor=$load_factor_val,
                    merk=$merk_val,nomor=$nomor_val,remarks=$remarks_val,vendor_id=$vendor_id_val,depricated=$depricated_val";
            
            	$sqlin="UPDATE equipment SET $cols_and_vals WHERE id=$id;";
	    ...
	    echo json_encode($msg);   
           ...
           $equipment=new Ref_Equipment_Model();
            ...
            $tpl->list_category =$list_category;
            $detail=$equipment->getEquipment($id);
			$tpl->detail = $detail;
	    	$tpl->url_add = url::current("add");
	    	$tpl->url_jsonData		= url::current("jsonData");
        	$tpl->url_comboAjax		=url::current("comboAjax");
...
public function del($id){     
     global $dcistem;
        ...
        if(trim($id)=="" or  $id == null){
	        ...
	        $Nama=$_POST['nama'];
	        $sqlin="DELETE FROM  equipment  WHERE id=$id;";
	        $rsl=$db->query($sqlin);
			if(isset($rsl->error) and $rsl->error===true){
		   	 		$msg['success']=false;
                	$msg['message']="Error, ".$rsl->query_last_message;
  ...
  public function validasiform($aksi="add",$kode_lama="") 
    {
        ...
        if(trim($_POST['category'])==''){
            $pesan["category"]="Kategori harus diisi!";   
            $msg[]="Kategori harus diisi!";
        }
    	if(trim($_POST['nomor'])==''){
            $pesan["nomor"]="Nomor body harus diisi!";   
            $msg[]="Nomor body harus diisi!";
        }
