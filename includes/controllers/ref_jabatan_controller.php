...
class Ref_Jabatan_Controller extends Admin_Template_Controller {
	...
	public function listdata() {
		...
		while($data = $db->fetchObject($list_qry))
		{
			...
			$ListData[$i]['nama_jabatan']=$data->nama_jabatan;
           ...
           if(trim($proses)=="save")
           {
            //    echo json_encode("tes");  
               ...
               if(count($validasi['arrayerror'])==0){
                   $cek=$db->select("kode","ref_jabatan")->where("kode='".$kode."'")->get(0);
                   if(empty($cek)){
						// $TglSkrg=date("Y-m-d H:i:s");
						$sqlin				="";
						$kode_val			=$master->scurevaluetable($kode);
						$nama_jabatan_val	=$master->scurevaluetable($nama_jabatan);

                       
                       $cols="kode,nama_jabatan";
                       $values="$kode_val,$nama_jabatan_val";
                       $sqlin="INSERT INTO ref_jabatan ($cols) VALUES ($values);";
	...
	public function edit($id,$proses=""){  
        ...
        {    
                    ...
                    if(count($validasi['arrayerror'])==0){
                        $cek=$db->select("kode","ref_jabatan")->where("kode='".$kode."'")->get(0);
                   		if(empty($cek)  || $kode == $id){
							...
							$cols_and_vals="kode=$kode_val,nama_jabatan=$nama_jabatan_val";
						
							$sqlin="UPDATE ref_jabatan SET $cols_and_vals WHERE kode=$id;";
            ...
            $partner=new Ref_Jabatan_Model();

            $tpl  = new View("form_ref_jabatan");
            $detail=$partner->getDetailRefJabatan($id);
            $tpl->detail = $detail;
    ...
    public function detail($id){     
		global $dcistem;
		$db   = $dcistem->getOption("framework/db");
		$tpl  = new View("detail_ref_jabatan");
		$insidenMdl    = new Ref_Jabatan_Model();
		date_default_timezone_set("Asia/Jakarta");
		$detail=$insidenMdl->getDetailRefJabatan($id);
		...
		$tpl->nama_jabatan       	= $detail->nama_jabatan;
		...
		$this->tpl->content_title = "Detail Ref Jabatan";
		$tpl->detail = $tpl;
		$tpl->render();   
 	} 
	...
	public function del($id){     
        ...
        if(trim($id)=="" or  $id == null){
            ...
            $sqlin="DELETE FROM  ref_jabatan  WHERE kode=$id;";
            $rsl=$db->query($sqlin);
            if(isset($rsl->error) and $rsl->error===true){
                    $msg['success']=false;
                    $msg['message']="Error, ".$rsl->query_last_message;
	...
	public function validasiform() 
    {
		...
		if(trim($_POST['nama_jabatan'])==''){
            $pesan["nama_jabatan"]="Nama Jabatan harus diisi!";   
            $msg[]="Nama Jabatan harus diisi!";
        }
		// if(trim($_POST['nik'])==''){
