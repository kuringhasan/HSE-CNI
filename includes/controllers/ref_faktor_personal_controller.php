...
class Ref_faktor_personal_Controller extends Admin_Template_Controller {
	...
	public function index() {
        ...
        $tpl  = new View("ref_faktor_personal");
	...
	public function listdata() {
		...
		while($data = $db->fetchObject($list_qry))
		{
			...
			$ListData[$i]['nama_faktor_personal']=$data->nama_faktor_personal;
        ...
        global $dcistem;
           ...
           if(trim($proses)=="save")
           {
            //    echo json_encode("tes");  
               ...
               $nama_faktor_personal	=trim($_POST['nama_faktor_personal']);

               

               $validasi=$this->validasiform();   
               if(count($validasi['arrayerror'])==0){
                   $cek=$db->select("kode","ref_faktor_personal")->where("kode='".$kode."'")->get(0);
                   if(empty($cek)){
						// $TglSkrg=date("Y-m-d H:i:s");
						$sqlin				="";
						$kode_val			=$master->scurevaluetable($kode);
						$nama_faktor_personal_val	=$master->scurevaluetable($nama_faktor_personal);

                       
                       $cols="kode,nama_faktor_personal";
                       $values="$kode_val,$nama_faktor_personal_val";
                       $sqlin="INSERT INTO ref_faktor_personal ($cols) VALUES ($values);";
			...
			$tpl  = new View("form_ref_faktor_personal");
	...
	public function edit($id,$proses=""){  
        ...
        {    
                    ...
                    if(count($validasi['arrayerror'])==0){
                        $cek=$db->select("kode","ref_faktor_personal")->where("kode='".$kode."'")->get(0);
                   		if(empty($cek) || $kode == $id){
							...
							$cols_and_vals="kode=$kode_val,nama_faktor_personal=$nama_faktor_personal_val";
						
							$sqlin="UPDATE ref_faktor_personal SET $cols_and_vals WHERE kode=$id;";
            ...
            $partner=new Ref_faktor_personal_Model();

            $tpl  = new View("form_ref_faktor_personal");
            $detail=$partner->getDetailRefFaktorPersonal($id);
            $tpl->detail = $detail;
    ...
    public function detail($id){     
		global $dcistem;
		$db   = $dcistem->getOption("framework/db");
		$tpl  = new View("detail_ref_faktor_personal");
		$insidenMdl    = new Ref_faktor_personal_Model();
		date_default_timezone_set("Asia/Jakarta");
		$detail=$insidenMdl->getDetailRefFaktorPersonal($id);
		// $tpl->list_contractor =Model::getOptionList("partner","id","name","name ASC","ifnull(is_contractor,0)=1"); 

		$tpl->kode            		= $detail->kode;
		$tpl->nama_faktor_personal       	= $detail->nama_faktor_personal;
	...
	public function del($id){     
        ...
        if(trim($id)=="" or  $id == null){
            ...
            $sqlin="DELETE FROM  ref_faktor_personal  WHERE kode=$id;";
            $rsl=$db->query($sqlin);
            if(isset($rsl->error) and $rsl->error===true){
                    $msg['success']=false;
                    $msg['message']="Error, ".$rsl->query_last_message;
	...
	public function validasiform() 
    {
		...
		if(trim($_POST['nama_faktor_personal'])==''){
            $pesan["nama_faktor_personal"]="Nama harus diisi!";   
