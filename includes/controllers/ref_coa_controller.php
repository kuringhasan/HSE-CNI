...
class Ref_Coa_Controller extends Admin_Template_Controller {
    ...
    public function listdata() {
       ...
       $draw=$_REQUEST['draw'];
        ...
        $list_qry=$db->select("SQL_CALC_FOUND_ROWS coa.id,coa.code,coa.name,odoo_id,coa.type, tp.name type_name","account_coa coa
        inner join account_type tp on tp.id=coa.type")
        ...
        while($data = $db->fetchObject($list_qry))
        {
            ...
            $ListData[$i]['ID']=$data->id;
             $ListData[$i]['Kode']=$data->code;
            $ListData[$i]['Nama']=$data->name;
            $ListData[$i]['Type']=$data->type_name;
            $ListData[$i]['OdooID']=$data->odoo_id;
...
public function refresh($id,$proses=""){     
    global $dcistem;
    $db   = $dcistem->getOption("framework/db");
    $master=new Master_Ref_Model();
    date_default_timezone_set("Asia/Jakarta");
