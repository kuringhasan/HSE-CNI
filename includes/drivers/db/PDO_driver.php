...
class PDO_Driver {
    			...
    			while($type = current($this->types)) {
    			     try {
        				if($this->config["persistent"]) {
            				$this->connection=new PDO("mysql:host=".$this->config["config"]["0"]["host"].";dbname=".$this->config["config"]["0"]["name"], $this->config["config"]["0"]["user"], 
                            $this->config["config"]["0"]["pass"],array(PDO::ATTR_PERSISTENT => true));
                            //$this->connection = @mysql_pconnect($this->config["config"]["0"]["host"], $this->config["config"]["0"]["user"], $this->config["config"]["0"]["pass"]);
            			} else {
            				$this->connection=new PDO("mysql:host=".$this->config["config"]["0"]["host"].";dbname=".$this->config["config"]["0"]["name"], $this->config["config"]["0"]["user"], 
                            $this->config["config"]["0"]["pass"]);
            			}
                    } catch (PDOException $e) {
                        Core::fatalError("Can't connect to MySQL server on '".$this->config["config"]["0"]["host"]."'. ".$e->getMessage());
	...
	public function query($sql) {
		...
		$result = $connection->query($sql);//@mysql_query($sql,$connection);
		$rec =new stdClass;
		if($result===false){
			global $dcistem;
			$rec->error=true;
