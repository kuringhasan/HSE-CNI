...
class DB_ODBC_MSSQL_Driver {
	...
	public function connect() {
		...
		case "double" :
			while($type = current($this->types)) {
				if($this->config["persistent"]) {
					...
					$this->connection[$type] = @odbc_connect("Driver={SQL Server};Server=".$this->config["config"][$type]["host"].";Database=".$this->config["config"][$type]["name"].";", $this->config["config"][$type]["user"], $this->config["config"][$type]["pass"]);
				} else {
		...
		default :
			if($this->config["persistent"]) {
				...
				$this->connection = @odbc_connect("Driver={SQL Server};Server=".$this->config["config"]["0"]["host"].";Database=".$this->config["config"]["0"]["name"].";", $this->config["config"]["0"]["user"], $this->config["config"]["0"]["pass"]);
			} else {
	...
	public function query($sql) {
		...
		$result = @odbc_exec($sql, $connection);
		if($result === false) {
			global $dcistem;
	...
	public function fetchObject($query) {
		...
		if(!empty($query)) {
			return @odbc_fetch_object($query);
		} else {
			return false;
		}
	}
		...
		if(!$this->config["persistent"]) {
			...
			case "double" :
				...
				while($type = current($this->types)) {
					...
					odbc_close($this->connection[$type]);
					next($this->types);
				}
				break;
			}
