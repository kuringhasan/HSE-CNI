...
class Core {
	...
	public function setOption($name, $value) {
		...
		$temp    = array();
		$part    = explode("/", $name);
		$temp[0] = $this->options[$part[0]];
		$end     = count($part) - 1;
		for($i=1;$i<=$end;$i++) {
			$temp[$i] = $temp[$i-1][$part[$i]];
	...
	private function startFramework() {
		global $dcistem;
		$db             = $dcistem->getOption("framework/db");
        $scriptname     = "/".trim(dirname((strlen($_SERVER["ORIG_SCRIPT_NAME"]) ? $_SERVER["ORIG_SCRIPT_NAME"] : $_SERVER["SCRIPT_NAME"])), "/");
        $scriptname    .= (substr($scriptname, -1) <> "/" ? "/" : "");
		//$pathinfo       = "/".trim((strlen($_SERVER["ORIG_PATH_INFO"]) ? $_SERVER["ORIG_PATH_INFO"] : $_SERVER["PATH_INFO"]), "/");
	...
	public static function error403() {
		...
		$tpl->title   = "Forbidden!";
		$tpl->message = "You are not authorized to view this page.";
	...
	public static function error($message = "") {
		...
		if(!$dcistem->getOption("framework/print/error")) {
			$content .= "
			<style>
				.core_error {color:#990000; border:1px solid #cc0000; font:12px Verdana; background:#ffcccc; padding:10px; -moz-border-radius:5px;}
			</style>";
		}
	...
	public static function benchmarkStart() {
		...
		$microtime        = microtime();
		list($sec,$micro) = split(" ", $microtime);
		$time             = $sec + $micro;
		$benchmark        = array(
								"memory" => $memory,
								"time"   => $time
