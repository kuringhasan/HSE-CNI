        ...
        <div class="box-body">
         ...
         <table id="list_data" class="table table-bordered table-hover dataTable"  style="width:100%">
            ...
            <tr>
              ...
              <th style="width:60px;text-align:center">Lokasi Absen</th>
  ...
  $j(document).ready(function() {
	$j('.select2').select2()
		...
		var table2=$j('#list_data').DataTable({
			...
			"columnDefs": [
				{ "orderable": false, "targets": [0,1,4,5,8]},
				{
					  "targets": [0,1,2,4,6,7,8], // your case first column
			...
			//console.log(aData);
				...
				$j( nRow ).find('td:eq(3)').attr('data-label',"Tanggal");
				$j( nRow ).find('td:eq(4)').attr('data-label',"Masuk");
				$j( nRow ).find('td:eq(5)').attr('data-label',"Pulang");
				$j( nRow ).find('td:eq(6)').attr('data-label',"Lokasi");
				$j( nRow ).find('td:eq(7)').attr('data-label',"Category");
