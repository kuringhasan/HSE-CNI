          ...
          <div class="box">
            ...
            <div class="box-body">
              ...
              <table id="list_data" class="table table-bordered table-hover dataTable"  style="width:100%">
                <thead>
                <tr>
                  <th rowspan="2" id="row_no" style="width:35px;">No</th>
                  <th rowspan="2" id="row_no" style="width:45px;">ID</th>
                  <th colspan="2" >Role</th>
                  <th colspan="2" style="width:250px;">Target Page</th>
                  <th rowspan="2" style="width:85px;">Aksi</th>
                </tr>
                <tr>
                  <th style="width:150px;" >Role ID</th>
                  <th style="width:180px;">Role Name</th>
                  <th style="width:45px;">Page ID</th>
                ...
                <tr>
                  ...
                  <th></th>
                  <th >
                  <input type="text" data-column="0"  class="form-control search-by-name" >
                  </th>
                  <th >&nbsp;</th>
                  <th >&nbsp;</th>
...
var $j = jQuery.noConflict();
  $j(document).ready(function() {
	...
	var table=$j('#list_data').DataTable( {
		...
		$j('.search-by-category').on( 'keyup click', function () {   // for text boxes
			var i =$j(this).attr('data-column');  // getting column index
			var v =$j(this).val();  // getting search input value
			table.columns(i).search(v).draw();
		} );	
		$j('.search-by-active').on( 'keyup click', function () {   // for text boxes
			var i =$j(this).attr('data-column');  // getting column index
			var v =$j(this).val();  // getting search input value
			table.columns(i).search(v).draw();
		} );	
		$j('#btn-tambah-data').on( 'click', function () {   // for text boxes
			var target = $j(this).attr('href');
		  	$j('#largeModal .modal-title').html("Tambah Package");
		...
		table.on('click', '.btn-edit-data', function (e) {
			var target = $j(this).attr('href');
		  	...
		  	$j('#largeModal .modal-title').html("Edit Package");
			$j('#largeModal .modal-body').load(target, function() {
				 $j('#largeModal').modal('show');
			});
			$j('#largeModal .modal-footer').html('');
