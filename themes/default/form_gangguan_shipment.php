...
$j(document).ready(function () {
	...
	$j("#gangguan_name").keyup(function(){
			var dt=$j(this).val()
			if (dt.length==0){
				$("#uneffective_id").val('');
			}
	}); 
	$j("#form_input_gangguan").on('keydown.autocomplete', '#gangguan_name', function () {
		//$j(".search-by-anggota").on("keydown.autocomplete", function(e) {
							//alert("<?php echo $url_jsonData."";?>/list_gangguan");								
			
				$j(this).typeahead({
					ajax: { 
						url: "<?php echo $url_jsonData;?>/list_gangguan",
						timeout: 500,
						displayField: "Nama",
						valueField :'ID',
						triggerLength: 1,
				   ...
				   onSelect: function(item) {
						...
						$j("#uneffective_id").val(item.value);
					}
				});
		});
...
<form action="" method="post" name="form_input_gangguan"  id="form_input_gangguan">

  
   
     <div class="row-form">
        <span class="label">Gangguan</span>
        
         <select name="uneffective_id" id="uneffective_id"  class="input" style=";">
          <?php
                echo '<option value="">--Uneffective Time--</option>';
                $List=$list_uneffective;
                while($data = each($List)) {
                    $gangguan_id=isset($_POST['uneffective_id'])?$_POST['uneffective_id']:$detail->gangguan_id;
                   ?>
          <option value="<?php echo $data['key'];?>"  <?php echo $data['key']==$gangguan_id?"selected":""; ?> > <?php echo $data['value'];?></option>
          <?php
               }
                ?>
          </select>
   ...
   <div class="row-form"><span class="label" >Shift</span>
    ...
    <div class="row-form">
				  ...
				  <textarea name="description" cols="35" rows="2" id="description"  class="input"  ><?php echo $detail->description;?></textarea>	
			   </div>
