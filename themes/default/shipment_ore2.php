        ...
        <div class="box-body">
         ...
         <table id="list_data" class="table table-bordered table-hover dataTable"  style="width:100%">
            ...
            <tr>
              ...
              <th style="width:70px;">Barge</th>
              <th style="width:55px;">Urutan</th>
              <th style="width:110px;">Commenced</th>
               <th style="width:110px;">Completed</th>
                <th style="width:60px;">Progress</th>
              <th style="width:70px;text-align:center">Total Ritase</th>
              <th style="width:75px;text-align:center">Final  Draught (MT)</th>
              <th style="width:60px;text-align:center">Verification</th>
	...
	$j('.select2').select2()
		...
		var table2=$j('#list_data').DataTable({
			...
			"columnDefs": [
				...
				{
					  "targets": [0,1,2,4,5,6,7,8], // your case first column
	  ...
	  '<input type="text" name="cr_tahun" class="form-control input-xm cr_tahun" id="cr_tahun2" onkeyup="pilih(this.value,8,\'cr_tahun\');" placeholder="tahun" size="4">\n';
		...
		//table2.removeClass("sorting_asc");
				...
				$j('#btn-tambah-data').on( 'click', function () {   // for text boxes
    				...
    				//$j("#media-create-report").attr("src", target);
				...
				table2.on('click', '.btn-edit-data', function (e) {
					...
					//alert('<?php echo $url_form;?>?id='+id);
					$j('#largeModal .modal-title').html("Laporan Shipment");
					$j('#largeModal .modal-body').load('<?php echo $url_form_step;?>?shipment_id='+id, function() {
