...
#list_data tr th{
 ...
 <div class="row">
            ...
            <div class="box-body">
              ...
              <table id="list_data" class="table table-bordered table-hover dataTable"  style="width:100%">
                ...
                <tr>
                  ...
                  <th  style="width:160px;">Parent ID</th>
        ...
        <select name="web_id" id="web_id" data-column="4" class="form-control input-xm search-by-webid" >
             ...
             <option value="ceria-staffs"  <?php echo "ceria-staffs"==$web_id?"selected":""; ?> >ceria-staffs</option>
                  ...
                  <th  > <input type="text" data-column="1"  class="form-control search-by-nomor" style="width:100%" placeholder="ID">
                  </th> 
                   <th  ></th>                  
                  <th  ></th>                  
                  <th  ></th>
...
var $j = jQuery.noConflict();
  $j(document).ready(function() {
	...
	var table=$j('#list_data').DataTable( {
					...
					"columnDefs": [
						{ "orderable": false, "targets": [0,3,4,5,6,7,9] },
						{"targets": [0,3,5,6,7],"className": "text-center"}
					 ],
					"columns": [
		...
		//var v =""; 
		$j('.search-by-nomor').on( 'keyup click', function () {   // for text boxes
			var i =$j(this).attr('data-column');  // getting column index
			var v =$j(this).val();  // getting search input value
			table.columns(i).search(v).draw();
		} );	
		$j('.search-by-webid').on( 'keyup click', function () {   // for text boxes
			var i =$j(this).attr('data-column');  // getting column index
			var v =$j(this).val();  // getting search input value
			table.columns(i).search(v).draw();
		} );	
		...
		$j('#btn-tambah-data').on( 'click', function () {   // for text boxes
			var target = $j(this).attr('href');
		  	$j('#largeModal .modal-title').html("Form Input Pagelist");
		...
		table.on('click', '.btn-edit-data', function (e) {
			var target = $j(this).attr('href');
		  	...
		  	$j('#largeModal .modal-title').html("Form Input Equipment");
			$j('#largeModal .modal-body').load(target, function() {
				 $j('#largeModal').modal('show');
			});
			$j('#largeModal .modal-footer').html('');
